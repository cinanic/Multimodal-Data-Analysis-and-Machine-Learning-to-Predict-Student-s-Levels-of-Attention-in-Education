...
def read_participants_Basic(datadir, user_list, pids, prune_length=None, aoifile=None,
    ...
    for rec, pid, offset in zip(user_list, pids, log_time_offsets):
        ...
        if params.EYETRACKERTYPE == "TobiiV2":
            ...
            allfile = "{dir}/Recovery_Participant_{rec}.tsv".format(dir=datadir, rec=rec)
            fixfile = "{dir}/Recovery_Participant_{rec}.tsv".format(dir=datadir, rec=rec)
            sacfile = "{dir}/Recovery_Participant_{rec}.tsv".format(dir=datadir, rec=rec)
            #evefile = "{dir}/Recovery_Participant_{rec}.tsv".format(dir=datadir, rec=rec)
            evefile = None
            segfile = "{dir}/Recovery_Participant_{rec}.seg".format(dir=datadir, rec=rec)
